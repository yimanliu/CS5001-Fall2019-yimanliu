...
def search_for_food(database):
    """
    Provide a list of keywords about food for user to search, one at a time.
    Lists the items from the database that match the keyword,
    and then lets the user choose one of the foods.
    Repeats the process until the user types "0" to indicate that they have
    ...
    for key, val in database['FOOD_DES'].items():
        food_item = val[1].split(" ")
        if len(food_item[0]) > 2:
            if food_item[0][-1] == ',':
                food_set.add(food_item[0][:-1])
            else:
                food_set.add(food_item[0])
    ...
    while True:
        ...
        search_idx = int(input(
            'Please choose a keyword about the food you want to search for '
            'from above list (0 to finish): '))
        if search_idx == 0:
            break
        matches = []
        for key, val in database['FOOD_DES'].items():
            if food_list[int(search_idx) - 1] in val[1]:
        ...
        choice = input("Which certain food would you like to add? "
                       "(<enter> for none): ")
        if choice == '':
            print()
            continue
