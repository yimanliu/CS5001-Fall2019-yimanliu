...
def graph_nutrients(database, foods, nutrient_list):
    """
    ...
    title = f"Serving: {foods[0]['weight'][0]} {foods[0]['weight'][1]}, " \
            f"Energy: {database['NUT_DATA'][(foods[0]['ndb_no'], 208)][0]}kCal"
    bargraph(title, 'value', nutrient_list, food_graph_dict)


def choose_nutrients():
    """
    ...
    """
    return ["Protein", "Carbohydrate, by difference", "Total lipid (fat)"]
...
def main():
    ...
    food = search_for_food(database)
    nutrients = choose_nutrients()
    graph_nutrients(database, food, nutrients)
